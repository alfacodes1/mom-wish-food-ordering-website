...
<div class="card">
  <div class="card-body">
    <h2 class="grid_title">Banner Master</h2>
            ...
            <tbody>
              ...
              if(mysqli_num_rows($res)>0) :
                ...
                while($row=mysqli_fetch_assoc($res)) : ?> 
                  <tr>
                    <td><?= $i ?></td>
                    <td><img src="<?= SITE_BANNER_IMAGE.$row['image']?>"></td>
                    <td><?= $row['heading']?></td>
                    <td><?= $row['sub_heading']?></td>
                    <td>
                        <a href="manage_banner.php?id=<?= $row['id'] ?>"><label class="badge badge-success hand_cursor">Edit</label></a>
                        &nbsp;
                        <?php
                        if($row['status']==1){
                          ?>
                          <a href="?id=<?= $row['id']?>&type=deactive"><label class="badge badge-danger hand_cursor">Active</label></a>
                          <?php
                        }else{
                          ?>
                          <a href="?id=<?= $row['id']?>&type=active"><label class="badge badge-info hand_cursor">Deactive</label></a>
                          <?php
                        }
                        ?>
                        &nbsp;
