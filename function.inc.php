...
function send_email($email,$html,$subject){
  $mail=new PHPMailer(true);
	...
	$mail->SMTPSecure="tls";
	$mail->SMTPAuth=true;
	$mail->Username="alfaalfi00001@gmail.com";
	$mail->Password="alfa00001alfi";
	$mail->setFrom("alfaalfi00001@gmail.com");
...
function manageUserCart($uid,$qty,$attr){
    global $con;
    $res=mysqli_query($con,"select * from dish_cart where user_id='$uid' and dish_details_id='$attr'");
    if(mysqli_num_rows($res)>0){
        ...
        mysqli_query($con,"insert into dish_cart(user_id,dish_details_id,qty,added_on) values('$uid','$attr','$qty','$added_on')");
    }
}
...
function getUserFullCart($attr_id=''){
    ...
    if(isset($_SESSION['FOOD_USER_ID'])){
        ...
        foreach($getUserCart as $list){
          ...
          $cartArr[$list['dish_details_id']]['price']=$getDishDetail['price'];
          $cartArr[$list['dish_details_id']]['dish']=$getDishDetail['dish'];
          $cartArr[$list['dish_details_id']]['image']=$getDishDetail['image'];
        }
    }else{
        if(isset($_SESSION['cart']) && count($_SESSION['cart'])>0){
            foreach($_SESSION['cart'] as $key=>$val){
...
function removeDishFromCartById($id){
    if(isset($_SESSION['FOOD_USER_ID'])){
        global $con;
        $res=mysqli_query($con,"delete from dish_cart where dish_details_id='$id' and user_id=".$_SESSION['FOOD_USER_ID']);
...
function getRatingByDishId($id){
  ...
  if($row['total']>0){
    ...
    echo "<span class='rating'> (".$arr[round($totalRate)]." -".$row['total']." users) </span>";
  }
}
function getSale($start,$end){
  global $con;
  $sql="select sum(total_price) as final_price from order_master where added_on between '$start' and '$end' and order_status=4";
  $res=mysqli_query($con,$sql);
